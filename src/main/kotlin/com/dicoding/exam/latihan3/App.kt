...
fun <T> checkType(args: T): String {
    val value =
        when (args) {
            ...
            // untuk pengecekan kondisi yang bawah ini aku bikin seperti ini karena di requirement diatas
            // mewajibkan tipe nilai List<String> & Map<String, String>
            is List<*> -> {
                if (args.all { it is String }) {
                    "Yes! it\'s List"
                } else {
                    "Yes! it\'s another type List"
                }
            }
            is Map<*, *> -> {
                if (args.all { it.key is String && it.value is String }) {
                    "Yes! it\'s Map"
                } else {
                    "Yes! it\'s another type Map"
                }
            }
